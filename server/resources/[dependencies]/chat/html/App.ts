...
import { Suggestion, SuggestionParam } from './Suggestions';
...
export default Vue.extend({
  ...
  methods: {
    ...
    getParamOptions(param: SuggestionParam | undefined, previousToken: string): string[] {
      if (!param) {
        return [];
      }

      const baseOptions = Array.isArray(param.options) ? param.options : [];
      const grouped = param.optionsByToken || {};
      const byPrevious = grouped[(previousToken || '').toLowerCase()] || [];

      const out: string[] = [];
      const seen: { [key: string]: boolean } = {};
      [...baseOptions, ...byPrevious].forEach((entry) => {
    ...
    setMessageWithCaret(nextValue: string) {
      ...
      this.$nextTick(() => {
        ...
        const caretPos = this.message.length;
        input.focus();
        input.setSelectionRange(caretPos, caretPos);
      });
    },
    applySuggestionCompletion(): boolean {
      const currentMessage = this.message || '';
      const activeSuggestion = this.getCurrentSuggestions().find((s) => !s.disabled);
      if (!activeSuggestion) {
        return false;
      }

      const commandName = activeSuggestion.name;
      if (!currentMessage.startsWith(commandName + ' ') && currentMessage !== commandName) {
        let nextCommandValue = commandName;
        if (!nextCommandValue.endsWith(' ')) {
          nextCommandValue += ' ';
        }
        this.setMessageWithCaret(nextCommandValue);
        return true;
      }

      const endsWithSpace = /\s$/.test(currentMessage);
      const parts = currentMessage.trim().split(/\s+/);
      ...
      const param = (activeSuggestion.params || [])[argIndex];
      const paramOptions = this.getParamOptions(param, previousToken);

      if (paramOptions.length > 0) {
        const lookup = currentToken.toLowerCase();
        const match = paramOptions.find((opt) => opt.toLowerCase().startsWith(lookup)) || (lookup === '' ? paramOptions[0] : '');
        if (match) {
          const nextArgs = args.slice();
          if (endsWithSpace) {
            nextArgs.push(match);
          } else if (nextArgs.length === 0) {
            nextArgs.push(match);
          } else {
            nextArgs[nextArgs.length - 1] = match;
          }

          this.setMessageWithCaret(`${commandName} ${nextArgs.join(' ')} `);
          return true;
        }
      }

      let nextValue = commandName;
      if (!nextValue.endsWith(' ')) {
        nextValue += ' ';
      }
      this.setMessageWithCaret(nextValue);
      return true;
    },
    keyUp() {
      this.resize();
    ...
    keyDown(e: KeyboardEvent) {
      if (e.which === 38 || e.which === 40) {
        ...
        if (!e.shiftKey && !e.altKey) {
          this.applySuggestionCompletion();
        } else {
          do {
            --this.modeIdx;
